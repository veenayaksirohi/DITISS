...
### 5.5 Comparison: WEP vs WPA vs WPA2 vs WPA3

| Feature | WEP | WPA (TKIP) | WPA2 (AES-CCMP) | WPA3 |
|---|---|---|---|---|
| Encryption | RC4, static key + IV | TKIP (RC4-based) | AES-CCMP | AES-CCMP (stronger), SAE |
| Key mgmt | Static pre-shared key | PSK or 802.1X | PSK or 802.1X | SAE (Personal), 192-bit (Enterprise) |
| Known flaws | Severely broken (IV reuse) | TKIP weaknesses | PSK vulnerable to dictionary attack | Resists offline PSK cracking (SAE) |
| Status | Obsolete, must not be used | Deprecated, avoid TKIP | Current minimum baseline | Recommended for new deployments |
...
**What Is a Honeypot?**
A decoy system or service designed to attract attackers by appearing vulnerable/valuable, gather intelligence on TTPs, and distract attackers from real production assets. Any interaction with a honeypot is, by design, suspicious.
...
| **Honeypot** | Deception & intel | Isolated/DMZ networks | Gathers TTPs, distracts attackers | Risk if not isolated; no direct blocking |
